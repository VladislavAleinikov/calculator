...
describe("Square root", () => {
  ...
  test("should 4 ^ 0.5 to equal 2", () => {
    expect(squareRoot.calculate(4)).toBe(2);
  });
  test("should (-5) ^ 0.5 to throw Error", () => {
    expect(() => { squareRoot.calculate(-5); }).toThrow("Can't take the root of a negative number.");
  });
  test("should 0 ^ 0.5 to equal 0", () => {
    expect(squareRoot.calculate(0)).toBe(0);
  });
  test("should 0.16 ^ 0.5 to equal 0.4", () => {
    expect(squareRoot.calculate(0.16)).toBe(0.4);
  });
});